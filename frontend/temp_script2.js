    ...
    (async () => {
        ...
        if (res.status === 200 && res.data.success) {
            currentUserId = res.data.data.user_id;
        }
    })();
        ...
        if (response.status === 200 && response.data.success) {
            ...
            container.innerHTML = expList.map(exp => `
                <article class="bg-surface rounded-2xl p-6 shadow-sm border border-outline-variant/10 hover:shadow-md transition-all flex flex-col gap-4">
                    ...
                    <div>
                        <h3 onclick="openExperienceDetail(${exp.experience_id})" class="font-headline-md text-headline-md text-on-surface mb-2 hover:text-secondary cursor-pointer transition-colors">${exp.title}</h3>
                        <p onclick="openExperienceDetail(${exp.experience_id})" class="font-body-md text-on-surface-variant line-clamp-2 cursor-pointer">${exp.content}</p>
                    </div>
    ...
    async function openExperienceDetail(id) {
        ...
        if (response.status === 200 && response.data.success) {
            ...
            document.getElementById('detailDescription').textContent = exp.content;
